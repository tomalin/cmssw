...
void DigitalTrack::checkAccuracy() const {
  if (accepted_) {  // Don't bother apply to tracks rejected by the fitter.
    ...
    constexpr float smallTA = 0.01, smallTB = 0.001, smallTC = 0.05, smallTD = 0.002, smallTE = 0.05, smallTF = 0.5, smallTG = 0.5;
    if (std::abs(TA) > smallTA || std::abs(TB) > smallTB || std::abs(TC) > smallTC || std::abs(TD) > smallTD || std::abs(TE) > smallTE || std::abs(TF) > smallTF || std::abs(TG) > smallTG) {
      throw cms::Exception("LogicError") << "WARNING: DigitalTrack lost precision: " << fitterName_ << " accepted=" << accepted_ << " " << TA << " " << TB << " " << TC << " " << TD << " " << TE << " " << TF << " " << TG;
    }
  }
}
