...
namespace Phase2Tracker {
  ...
  Phase2TrackerFEDFEDebug::Phase2TrackerFEDFEDebug(FEDReadoutMode readoutmode, READ_MODE debugmode)
      : _fedebugstatus(0), _chipdebugstatus(), _is_on(false), _readoutmode(readoutmode), _debugmode(debugmode) {}
  ...
  uint16_t Phase2TrackerFEDFEDebug::getChipL1ID(uint8_t chip_id) const {
    if (_readoutmode == READOUT_MODE_PROC_RAW && _debugmode == FULL_DEBUG) {
      ...
      LogTrace("Phase2TrackerFEDFEDebug")
          << "Warning : Chip L1Id only present in Unsparsified, Full debug mode" << std::endl;
#endif
    }
    return 0;
  }
  ...
  std::vector<uint16_t> Phase2TrackerFEDFEDebug::getFEL1ID() const {
    ...
    if (_readoutmode == READOUT_MODE_ZERO_SUPPRESSED && _debugmode == FULL_DEBUG) {
      ...
      LogTrace("Phase2TrackerFEDFEDebug") << "[Phase2Tracker::Phase2TrackerFEDFEDebug::" << __func__ << "]: \n";
      LogTrace("Phase2TrackerFEDFEDebug")
          << "Warning : FE L1Id only present in Unsparsified, Full debug mode" << std::endl;
#endif
  ...
  uint16_t Phase2TrackerFEDFEDebug::getChipPipelineAddress(uint8_t chip_id) const {
    if (_readoutmode == READOUT_MODE_PROC_RAW && _debugmode == FULL_DEBUG) {
      ...
      LogTrace("Phase2TrackerFEDFEDebug") << "[Phase2Tracker::Phase2TrackerFEDFEDebug::" << __func__ << "]: \n";
      LogTrace("Phase2TrackerFEDFEDebug")
          << "Warning : Chip Pipeline Address only present in Unsparsified, Full debug mode" << std::endl;
#endif
    }
    return 0;
  }

  void Phase2TrackerFEDFEDebug::setFEDebugStatus(uint32_t fe_status) { _fedebugstatus = fe_status; }

  void Phase2TrackerFEDFEDebug::setReadoutMode(FEDReadoutMode readoutmode) { _readoutmode = readoutmode; }

  void Phase2TrackerFEDFEDebug::setDebugMode(READ_MODE debugmode) { _debugmode = debugmode; }
